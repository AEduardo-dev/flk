...
pub fn direnv_init() -> Result<()> {
    ...
    let direnv_content = "use flake --flake flk.nix --impure ";
...
pub fn direnv_attach() -> Result<()> {
    ...
    direnv_content.push_str("\nuse flake --flake flk.nix --impure ");
...
pub fn direnv_detach() -> Result<()> {
    ...
    let updated_content: String = direnv_content
        .lines()
        .filter(|line| !line.contains("use flake --flake flk.nix --impure"))
        .map(|line| format!("{}\n", line))
        .collect();
...
mod tests {
    ...
    fn test_direnv_attach() {
        ...
        let original_dir = std::env::current_dir().unwrap();
        std::env::set_current_dir(&dir).unwrap();
        fs::write(".envrc", "export VAR=value").unwrap();
        let result = direnv_attach();
        assert!(result.is_ok());
        let direnv_path = Path::new(".envrc");
        assert!(direnv_path.exists());
        let content = fs::read_to_string(direnv_path).unwrap();
        assert!(content.contains("export VAR=value"));
        assert!(content.contains("use flake --flake flk.nix --impure"));
        std::env::set_current_dir(original_dir).unwrap();
    }
    #[test]
