...
refresh() {{
  ...
  if _flk_use_direnv; then
    ...
    exec nix develop "$_flk_ref" --impure --profile ".flk/.nix-profile-$_flk_profile_name" -c "$SHELL"
  fi
}}
...
switch() {{
  ...
  if _flk_use_direnv; then
    ...
    exec nix develop ".#$profile" --impure --profile ".flk/.nix-profile-$profile" -c "$SHELL"
 fi
}}
"#
    );
...
function refresh --description "Reload env (direnv if present, else nix develop)"
  if _flk_use_direnv
    ...
    exec nix develop "$flk_ref" --impure --profile ".flk/.nix-profile-$profile_name" -c "$SHELL"
  end
end
